...
MLP with architecture 49 ¨C 30 ¨C 1. Learning rate = 0.04, activation function = ¡®relu¡¯, stopping until convergence.
...
Multilayer feed forward Neural network: 
I use the SK-Learn ¡®OpenCV¡¯, ¡®ShuffleSplit¡¯, ¡®MLP¡¯ and ¡®train_test_split¡¯ methods to support my thinking.
Procedures:
1.	Load data and preprocessing the data:
...
Although we know that the range of this function is [-¡Þ£¬¡Þ], However GP cannot using this as search space, because, it is quite big. In this problem, I set 1000 numbers belongs 
...
into one feature and then use Na?ve Bayes and Decision Tree classify algorithms to solve classification which could using cross validation to find the proper accuracy.
